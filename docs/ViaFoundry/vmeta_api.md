...
This guide is aimed at introducing users to the V-Meta API, with which one can interact with V-Meta through the command line as opposed to the console/user interface. You'll find sample requests and responses related to [projects](vmeta_api.md#projects), [collections](vmeta_api.md#collections), [fields](vmeta_api.md#fields), and [data](vmeta_api.md#data).

The V-Meta API utilizes REST (Representational State Transfer) architecture for its communication. All API requests pull information from HTML pages, and all responses, including errors, are returned in JSON format. HTTP response status codes are used to indicate the success or failure of the API calls.

## Authentication and authorization

Requests to the V-Meta API are for both public and private information, so all
endpoints require authentication.

### User Login

After getting their access token, users can make a request to the V-Meta
...
Here's how V-Meta API users can retrieve an access token for the currently logged-in user.
...
In V-Meta, projects serve as your analysis hubs, acting as silos for your data and collections. Some important and frequently-used API requests include retrieving a user's project(s), as well as creating, updating, and deleting projects.
